class SoftwareStructure3 {
   ...
   *
   * This implementation by Russell Gordon, 2014.
   *
   * License for use:
   *
  ...
  boolean drawIntersections = true;
  boolean drawConnections = true;
  boolean drawConnectionCircles = false;
  ...
  void keyPress() {
    ...
    if (key == 'a') {
      ...
      if (drawConnectionCircles) {
        drawConnectionCircles = false;
      } else {
        drawConnectionCircles = true;
      }
    } else if (key == 's') {
      circles = 3;
      mySetup();
  ...
  void drawIntersections(int a, int b) {
    ...
    // Draw a circle with the length between intersections as the diameter
    if (drawConnectionCircles) {
      float midX = (I1.x + I2.x) / 2;
      float midY = (I1.y + I2.y) / 2;
      float diameter = I1.dist(I2);
      noStroke();
      if (aggregate) {
        fill(((opacity*6)*3.6*1.5) % 360, 80, 100, opacity);
      } else {
        fill(((opacity)*3.6*1.5) % 360, 80, 100, opacity);
      }
